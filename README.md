...
不同支付页版式差异很大，OCR 有时会漏掉大号金额，或把 `Transaction successful` 等状态文案误当成商户。LedgerPilot 会先按微信/支付宝交易详情页模板提取字段，再用通用规则兜底：

- 如果金额没有被 OCR 识别出来，记录不会被直接确认入账；App 会提示“金额未识别，请补充”。
- 如果你在 App 内选择/粘贴截图，整图 OCR 没识别到金额时，App 会自动裁剪截图中常见金额区域做一次数字专用二次 OCR。
- 如果页面是支付宝英文 `Transaction Details` 版式，App 会优先从 `Transaction successful` 上方的商户行提取商户。
- 如果商户文字被 OCR 拆成空格，例如 `探 鱼 : 鲜 青 椒...`，App 会自动压缩成正常商户名。
- 如果分类失败，可以在确认面板里改分类；常见餐饮关键词如 `探鱼`、`烤鱼`、`Food` 会自动归入餐饮。

遇到金额漏识别时，优先尝试在 App 内直接选择原截图，或停留在金额更清晰的支付成功页/订单详情页再触发背部轻点；仍漏识别时手动补金额即可。注意：如果快捷指令只把 OCR 文本传给 App，而没有把原图交给 App，App 无法对原图做二次裁剪识别。
